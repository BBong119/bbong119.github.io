...
`DeformationResistingModes` is a [`DeformationResistingMode`]({{ site.manual_interface_enum }}result-enum.html#intermediateresultsavingmode) array defines in `furtherModes` which is a struct member of `PublicRuntimeSettings`. It is used for setting the modes and the priority for deformation resisting. Default value will be used if there is no manual setting.
